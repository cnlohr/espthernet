...
uint8_t et_TCPReceiveSyn( uint16_t portno );

//Return 1 if you DID emit a packet (with ack) Return 0 if you JUST received data and we need to 
uint8_t et_TCPReceiveData( uint8_t connid, uint16_t totallen );
...
void    ICACHE_FLASH_ATTR et_TCPConnectionClosing( uint8_t conn );
...
int8_t et_GetFreeConnection();
void et_RequestClosure( uint8_t c );
void et_RequestConnect( uint8_t c );
uint8_t et_TCPCanSend( uint8_t c );
...
void ICACHE_FLASH_ATTR et_EmitTCP( uint8_t );
